                            ...
                            val newMessages =
                                snapshot.children.mapNotNull { postSnapshot ->
                                    ...
                                    when (val type = postSnapshot.child(MessageVal.TYPE).value.toString()) {
                                        ...
                                        MessageVal.PHOTO -> {
                                            val photoUri =
                                                postSnapshot.child(MessageVal.PHOTO).value.toString().let(
                                                    Uri::parse)
                                            Message.PhotoMessage(postSnapshot.key.toString(), photoUri, user, timestamp)
                                        }
                                        MessageVal.FILE -> {
                                            val fileUri =
                                                postSnapshot.child(MessageVal.FILE).value.toString().let(
                                                    Uri::parse)
                                            Message.FileMessage(postSnapshot.key.toString(), fileUri, user, timestamp)
                                        }
                                        MessageVal.LINK -> {
                                            val linkUri =
                                                postSnapshot.child(MessageVal.LINK).value.toString().let(
                                                    Uri::parse)
