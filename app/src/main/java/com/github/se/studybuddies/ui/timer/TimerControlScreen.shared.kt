...
fun TimerScreen(groupId: String, viewModel: SharedTimerViewModel) {
  ...
  Scaffold(topBar = { TopAppBar(title = { Text("Timer for Group $groupId") }) }) { paddingValues ->
    Column(
...
fun TimeAdjustButton(
    ...
    adjustment: Long,
    groupId: String,
    viewModel: SharedTimerViewModel
) {
  Button(
      onClick = { viewModel.adjustTime(groupId, adjustment) }, modifier = Modifier.padding(4.dp)) {
        Text(label)
      }
}
...
fun TimerControlButtons(groupId: String, viewModel: SharedTimerViewModel) {
  Row(horizontalArrangement = Arrangement.SpaceEvenly, modifier = Modifier.fillMaxWidth()) {
    Button(onClick = { viewModel.startTimer(groupId) }, modifier = Modifier.padding(4.dp)) {
      Text("Start")
    }
    Button(onClick = { viewModel.pauseTimer(groupId) }, modifier = Modifier.padding(4.dp)) {
      Text("Pause")
    }
    Button(onClick = { viewModel.resetTimer(groupId) }, modifier = Modifier.padding(4.dp)) {
      Text("Reset")
    }
  }
}
